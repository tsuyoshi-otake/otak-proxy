    ...
    test('uses the shared high-load command timeout', () => {
        ...
        test('setProxy writes only the http.proxy routing key', async () => {
            ...
            assert.deepStrictEqual(calls.map(call => call.args), [
                ['config', '--global', GIT_ROUTING_PROXY_KEY, 'http://proxy.example.com:8080']
            ]);
            assert.ok(!calls.some(call => call.args.includes(GIT_LEGACY_NON_ROUTING_PROXY_KEY)));
        });
        ...
        test('unsetProxy still clears leftover https.proxy', async () => {
            ...
            assert.deepStrictEqual(calls, [
                ['config', '--global', '--unset', GIT_ROUTING_PROXY_KEY],
                ['config', '--global', '--unset', GIT_LEGACY_NON_ROUTING_PROXY_KEY]
            ]);
        });
    });
